...
def _get_sentiment_batch(model, batch: list, current_batch_index: int, attempt=1):
    ...
    try:
        ...
        if len(scores) != len(batch):
            ...
            return "RETRY" if attempt < MAX_RETRIES else [0.0] * len(batch)
        return scores
    except Exception as e:
        print(f"API 에러: {e} (재시도 {attempt}/{MAX_RETRIES}) - Batch: {current_batch_index}")
        return "RETRY" if attempt < MAX_RETRIES else [0.0] * len(batch)


def analyze_descriptions(texts: list) -> list:
    print("\nGemini API로 뉴스 본문 감성 점수 분석을 시작합니다...")
    load_dotenv()
    ...
    for i in tqdm(range(0, len(texts), BATCH_SIZE), desc="Initial Labeling"):
        ...
        scores = _get_sentiment_batch(model, batch, idx, 1)
        if scores == "RETRY":
            batches_to_retry.append({"idx": idx, "data": batch, "attempt": 1})
            all_scores.extend(["PENDING"] * len(batch))
        else:
    ...
    while batches_to_retry:
        ...
        failed_batches = batches_to_retry[:]
        batches_to_retry = []
        for item in tqdm(failed_batches, desc=f"Retrying Pass {retry_pass}"):
            new_attempt = item["attempt"] + 1
            time.sleep(API_REQUEST_DELAY * (RETRY_DELAY_MULTIPLIER ** item["attempt"]))
            retried_scores = _get_sentiment_batch(model, item["data"], item["idx"], new_attempt)

            start_pos = item["idx"] * BATCH_SIZE
            if retried_scores == "RETRY":
                if new_attempt < MAX_RETRIES:
                    item['attempt'] = new_attempt
                    batches_to_retry.append(item)
                else:  # 최종 실패
                    for k in range(len(item["data"])): all_scores[start_pos + k] = 0.0
            else:  # 재처리 성공
                for k, score in enumerate(retried_scores): all_scores[start_pos + k] = score

    return [0.0 if s == "PENDING" else s for s in all_scores]